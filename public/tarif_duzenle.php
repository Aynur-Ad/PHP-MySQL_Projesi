...
$tarif_malzemeleri = $tarifObj->getTarifMalzemeler($tarif_id); // Mevcut tarifin malzemelerini çek
$tum_malzemeler = $malzemeObj->getMalzemeler(); // Tüm malzemeleri çek (seçim için)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $servis_sayisi = isset($_POST['servis_sayisi']) ? (int)$_POST['servis_sayisi'] : 1;
    $malzemeler_post = isset($_POST['malzemeler']) ? $_POST['malzemeler'] : []; // Formdan gelen malzemeler

    // Malzeme verilerini temizle ve filtrele
    $gecerli_malzemeler = [];
    foreach ($malzemeler_post as $malzeme) {
        $malzeme_id = (int)$malzeme['malzeme_id'];
        $miktar = trim($malzeme['miktar']);
    ...
    if (empty($ad) || empty($aciklama) || empty($hazirlik_suresi) || empty($pisirme_suresi) || $servis_sayisi <= 0) {
        ...
        if ($tarifObj->updateTarif($tarif_id, $_SESSION['user_id'], $ad, $aciklama, $hazirlik_suresi, $pisirme_suresi, $servis_sayisi, $gecerli_malzemeler)) {
            header("Location: tarifler.php?success=" . urlencode("Tarif başarıyla güncellendi!"));
            exit();
        } else {
            $error_message = "Tarif güncellenirken bir hata oluştu.";
        }
    }
}
?>

<h1 class="mb-4">Tarif Düzenle: <?php echo htmlspecialchars($tarif['ad']); ?></h1>
...
<div class="card p-4">
    <form action="tarif_duzenle.php?id=<?php echo $tarif_id; ?>" method="POST">
        <div class="mb-3">
            <label for="ad" class="form-label">Tarif Adı</label>
            <input type="text" class="form-control" id="ad" name="ad" required value="<?php echo htmlspecialchars($tarif['ad']); ?>">
        </div>
        <div class="mb-3">
            <label for="aciklama" class="form-label">Açıklama/Hazırlanışı</label>
            <textarea class="form-control" id="aciklama" name="aciklama" rows="5" required><?php echo htmlspecialchars($tarif['aciklama']); ?></textarea>
        </div>
        <div class="row mb-3">
            <div class="col-md-4">
                <label for="hazirlik_suresi" class="form-label">Hazırlık Süresi</label>
                <input type="text" class="form-control" id="hazirlik_suresi" name="hazirlik_suresi" required value="<?php echo htmlspecialchars($tarif['hazirlik_suresi']); ?>">
            </div>
            <div class="col-md-4">
                <label for="pisirme_suresi" class="form-label">Pişirme Süresi</label>
                <input type="text" class="form-control" id="pisirme_suresi" name="pisirme_suresi" required value="<?php echo htmlspecialchars($tarif['pisirme_suresi']); ?>">
            </div>
             <div class="col-md-4">
                <label for="servis_sayisi" class="form-label">Servis Sayısı</label>
                <input type="number" class="form-control" id="servis_sayisi" name="servis_sayisi" required min="1" value="<?php echo htmlspecialchars($tarif['servis_sayisi']); ?>">
            </div>
        </div>
        ...
        <div id="malzeme_listesi" class="mb-3">
            <?php
            $current_malzemeler = !empty($_POST['malzemeler'] ?? []) ? $_POST['malzemeler'] : $tarif_malzemeleri;

            if (empty($current_malzemeler)) {
                // Eğer hiç malzeme yoksa, varsayılan bir boş alan ekle
                ?>
                ...
                foreach ($current_malzemeler as $index => $malzeme_data) {
        ...
        <div class="d-grid gap-2 mt-4">
            <button type="submit" class="btn btn-success btn-lg">Tarifi Güncelle</button>
            <a href="tarifler.php" class="btn btn-secondary btn-lg">Geri Dön</a>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    let malzemeIndex = <?php echo count($current_malzemeler) > 0 ? count($current_malzemeler) : 1; ?>; // Malzeme satırı için sayaç
